...
export default function NearbyDropPointsScreen() {
  ...
  const handleCall = (phone) => Linking.openURL(phone);
  ...
  const filteredPoints = dropPoints.filter((point) => {
    if (selectedFilter === 'Open Now') return point.status === 'Open';
    if (selectedFilter === '< 5km') return parseFloat(point.distance) < 5;
    return true;
  });
          ...
          <View key={point.id} style={styles.card}>
            ...
            <View style={styles.actions}>
              <View style={styles.actionBtns}>
                ...
                <TouchableOpacity style={styles.directionBtn} onPress={() => handleDirection(point)}>
...
const styles = StyleSheet.create({
  ...
  activeTab: { backgroundColor: '#836EFE' },
  inactiveTab: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#E0E0E0' },
  activeTabText: { color: '#fff', fontWeight: '600', marginLeft: 6 },
  inactiveTabText: { color: '#836EFE', fontWeight: '600', marginLeft: 6 },
  filterRow: {
    flexDirection: 'row',
  ...
  filterBtnActive: { backgroundColor: '#836EFE' },
  filterText: { fontSize: 13, fontWeight: '500', color: '#836EFE' },
  ...
  map: { flex: 1 },
  cardScroll: { marginTop: 10, paddingHorizontal: 16 },
  card: {
    backgroundColor: '#fff',
    borderRadius: 14,
    padding: 14,
  ...
  name: { fontWeight: 'bold', fontSize: 16, color: '#333', marginRight: 6 },
  ratingBox: { flexDirection: 'row', alignItems: 'center', marginLeft: 'auto' },
  rating: { fontWeight: '600', fontSize: 13, color: '#000' },
  status: {
    fontSize: 12,
    paddingHorizontal: 8,
    paddingVertical: 2,
  ...
  open: { backgroundColor: '#D2F6DC', color: 'green' },
  closed: { backgroundColor: '#FDDCDC', color: 'red' },
  detailRow: { flexDirection: 'row', alignItems: 'center', marginTop: 2 },
  iconSpacing: { marginRight: 6 },
  address: { color: '#666' },
  distance: { color: '#888', fontSize: 12 },
  time: { color: '#444', fontSize: 12, marginBottom: 8 },
  actions: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 12 },
  actionBtns: { flexDirection: 'row', gap: 10 },
  callBtn: { borderWidth: 1, borderColor: '#ccc', borderRadius: 8, paddingVertical: 6, paddingHorizontal: 14 },
  directionBtn: { backgroundColor: '#836EFE', paddingVertical: 6, paddingHorizontal: 14, borderRadius: 8 },
  pinBox: { backgroundColor: '#EFE9FF', padding: 10, borderRadius: 12 },
  infoRowWithPin: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginTop: 6 },
